(() => {
    ...
    // NOVO: Um 'Set' para armazenar as URLs existentes. É muito mais rápido para buscas.
    let existingUrls = new Set();

    const h = document.getElementById("pauseBtn"),
        y = document.getElementById("resumeBtn");

    // NOVO: Função para buscar as URLs existentes do servidor
    async function fetchExistingUrls() {
        try {
            // Ajuste o caminho se o seu arquivo PHP estiver em outro lugar
            const response = await fetch('./api/controles/get_existing_urls.php');
            if (!response.ok) {
                throw new Error('Falha ao buscar a lista de URLs existentes.');
            }
            const data = await response.json();
            if (data.urls && Array.isArray(data.urls)) {
                // Armazena as URLs no Set
                existingUrls = new Set(data.urls);
                console.log(`Pronto! ${existingUrls.size} URLs existentes foram carregadas.`);
            }
        } catch (error) {
          alert(error)
            console.error("Erro ao buscar URLs existentes:", error);
            Swal.fire({
                title: "Erro de Sincronização",
                text: "Não foi possível carregar a lista de conteúdo existente. A verificação de duplicados não funcionará como esperado.",
                icon: "warning",
    ...
    // MODIFICADO: Esta função agora filtra o conteúdo duplicado antes de retorná-lo.
    function U(e) {
        const t = e.split("\n");
        let o = []; // Array para conteúdo NOVO
        let n = !1;
        
        let initialDuplicateCount = p; // Guarda a contagem inicial de duplicados
        ...
        for (let e = 0; e < t.length; e++) {
            ...
            if (r.startsWith("#EXTINF")) {
                ...
                // MODIFICADO: A verificação principal acontece aqui!
                if (u && $(u)) {
                    // Se a URL já existir no nosso Set, ignora e contabiliza como duplicado.
                    if (existingUrls.has(u)) {
                        p++; // Incrementa o contador de conteúdo duplicado
                    } else {
                        // Se for nova, adiciona ao array 'o' para ser enviada.
                        o.push({
                            url: u,
                            groupTitle: i,
                            tvgLogo: s,
                        ...
                        a++; // Incrementa o total de URLs a serem processadas
                    }
                }
                n = !1
            }
        ...
        let duplicatesFoundInThisPart = p - initialDuplicateCount;
        console.log(`Itens novos a serem enviados: ${o.length}. Duplicados ignorados nesta parte: ${duplicatesFoundInThisPart}`);
        
        // Atualiza o contador de duplicados na interface
        document.getElementById("exitente").textContent = p;
        
        // Retorna APENAS o conteúdo novo
        return o;
    }

    // MODIFICADO: Esta função agora lida com a lista já filtrada.
    async function O(e) {
        D.hide();
        for (let t = 0; t < e.length; t++) {
            // Primeiro, processamos e filtramos o conteúdo da parte atual do arquivo.
            const newContent = U(e[t]);

            // Se, após a filtragem, não sobrar nenhum item novo, simplesmente pulamos para a próxima parte.
            if (newContent.length === 0) {
                console.log(`Parte ${t + 1} do arquivo não continha itens novos. Pulando.`);
                continue; // Pula para a próxima iteração do loop
            }

            // Se houver itens novos, nós os dividimos em blocos e os enviamos.
            const n = T(newContent, qnturl);
            b = n;
        ...
        // A lógica de finalização só é chamada após todas as partes serem processadas.
        B = e.length, w = 0, 0 == o && (await H(), o = !0);
    ...
    async function j(e, t, o) {
        ...
        // NOVO: Chama a função para buscar as URLs existentes assim que o script é carregado
        fetchExistingUrls();
